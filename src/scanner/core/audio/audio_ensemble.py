...
DEFAULT_WEIGHTS = {
    "wavlm_forensic": 0.25, "cqt_spectral": 0.25, "ecapa_tdnn": 0.15,
    "voice_clone": 0.15, "syncnet_av": 0.20,
}
    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        ...
        if not sub:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="audio_ensemble_skip",
        ...
        return DetectorResult(
            ...
            method="audio_weighted_ensemble", status=DetectorStatus.PASS,
