...
class CQTDetector(BaseDetector):
    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        if inp.audio_waveform is None:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="cqt_skip",
                                  status=DetectorStatus.SKIPPED)
        sr = inp.audio_sr or 16000
        cqt = self._compute_cqt(inp.audio_waveform, sr)
        if cqt is None:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.1, method="cqt_fallback",
        ...
        return DetectorResult(
            ...
            score=score, confidence=0.65, method="cqt_spectral_analysis",
            status=DetectorStatus.PASS,
            details={"flatness": round(flatness, 4), "bandwidth": round(bw, 4),
                     "temporal_var": round(tv, 4)})
    ...
    def _spectral_bandwidth(spec: np.ndarray) -> float:
        ...
        bw = np.sqrt(np.sum(((freqs[:, None] - centroid) ** 2) * lin, axis=0)
                     / (np.sum(lin, axis=0) + 1e-10))
