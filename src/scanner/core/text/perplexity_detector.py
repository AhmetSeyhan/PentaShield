    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        if not inp.text or len(inp.text.strip()) < 50:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="ppl_skip",
                                  status=DetectorStatus.SKIPPED)
        if self.model is None:
            score, details = self._entropy_fallback(inp.text)
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=score, confidence=0.3, method="entropy_fallback",
                                  status=DetectorStatus.PASS, details=details)
        try:
            import torch
            ...
            with torch.no_grad():
                ...
                token_losses = loss_fn(shift_logits.view(-1, shift_logits.size(-1)),
                                       shift_labels.view(-1))
            ...
            return DetectorResult(
                ...
                score=score, confidence=0.7, method="gpt2_perplexity",
                status=DetectorStatus.PASS,
                details={"perplexity": round(ppl, 2), "ppl_std": round(ppl_std, 2),
                         "n_tokens": len(token_ppls)})
        except Exception as exc:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="ppl_error",
