...
class FrequencyDetector(BaseDetector):
    ...
    def capabilities(self) -> set[DetectorCapability]:
        return {DetectorCapability.VIDEO_FRAMES, DetectorCapability.SINGLE_IMAGE,
                DetectorCapability.FREQUENCY_ANALYSIS}
    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        ...
        if not frames:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="freq_skip",
        ...
        return DetectorResult(
            ...
            score=avg, confidence=min(0.85, 0.3 + 0.1 * len(analyses)),
            method="dct_dft_spectral", status=DetectorStatus.PASS,
            details={"n_frames": len(analyses),
    ...
    def _analyze_frame(self, frame: np.ndarray) -> dict[str, float]:
        ...
        return {"hf_ratio": float(hf_ratio), "flatness": float(flatness),
                "fake_score": float(max(0.0, min(1.0, fake)))}
