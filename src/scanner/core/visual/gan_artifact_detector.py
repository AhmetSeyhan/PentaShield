...
class GANArtifactDetector(BaseDetector):
    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        ...
        if not frames:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="gan_skip",
                                  status=DetectorStatus.SKIPPED)
        scores = [self._analyze(f) for f in frames[:8]]
        avg = float(np.mean(scores))
        return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                              score=avg, confidence=min(0.8, 0.3 + 0.1 * len(scores)),
                              method="gan_artifact_analysis", status=DetectorStatus.PASS,
                              details={"n_frames": len(scores)})

    def _analyze(self, frame: np.ndarray) -> float:
        return (0.4 * self._checkerboard(frame) + 0.3 * self._color_bleed(frame)
                + 0.3 * self._boundary(frame))
    ...
    def _checkerboard(frame: np.ndarray) -> float:
        ...
        h, w = mag.shape
        cy, cx = h // 2, w // 2
        corner = (mag[:h//8, :w//8].mean() + mag[:h//8, -w//8:].mean() +
                  mag[-h//8:, :w//8].mean() + mag[-h//8:, -w//8:].mean()) / 4
    ...
    def _color_bleed(frame: np.ndarray) -> float:
        ...
        diff = (np.abs(edges[0].astype(float) - edges[1].astype(float)).mean() +
                np.abs(edges[0].astype(float) - edges[2].astype(float)).mean()) / 2
        return float(min(1.0, diff / 10.0))

    @staticmethod
    def _boundary(frame: np.ndarray) -> float:
        gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY).astype(np.float32) if frame.ndim == 3 \
