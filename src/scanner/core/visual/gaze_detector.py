...
class GazeDetector(BaseDetector):
    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        ...
        return DetectorResult(
            ...
            score=score, confidence=min(0.7, 0.2 + 0.02 * len(gaze_points)),
            method="gaze_analysis", status=DetectorStatus.PASS,
    ...
    def _smoothness(points: list[tuple[float, float]]) -> float:
        if len(points) < 3:
            return 0.5
        vels = [np.sqrt((points[i][0]-points[i-1][0])**2 + (points[i][1]-points[i-1][1])**2)
    ...
    def _saccade_pattern(points: list[tuple[float, float]]) -> float:
        if len(points) < 4:
            return 0.5
        vels = np.array([np.sqrt((points[i][0]-points[i-1][0])**2 + (points[i][1]-points[i-1][1])**2)
                         for i in range(1, len(points))])
