...
DEFAULT_WEIGHTS = {
    "clip_forensic": 0.25, "efficientnet_b0": 0.15, "xception": 0.12,
    "vit_forgery": 0.10, "frequency_analysis": 0.10, "gan_artifact": 0.08,
    "diffusion_artifact": 0.08, "ppg_biosignal": 0.06, "gaze_consistency": 0.06,
}
    ...
    async def _run_detection(self, inp: DetectorInput) -> DetectorResult:
        ...
        if not sub_results:
            return DetectorResult(detector_name=self.name, detector_type=self.detector_type,
                                  score=0.5, confidence=0.0, method="ensemble_skip",
        ...
        return DetectorResult(
            ...
            score=final, confidence=min(0.95, float(np.mean(confs)) * 1.1) if confs else 0.0,
            method="weighted_ensemble", status=DetectorStatus.PASS,
